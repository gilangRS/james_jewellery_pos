...
namespace Connection.Repositories
{
    public class PackagingRepository : IPackagingRepository
    {
        ...
        public List<object> GetStockPackagingSummary(int Tipelokasi, int IDLokasi, int Packaging, string From, string To, int Groupby, bool IsStore, string NIK)
        {
            List<object> datas = new List<object>();
            string query = "exec sp_summary_packaging "
                + Tipelokasi + ", '"
                + From + "', '"
                + To + "', "
                + IDLokasi + ", "
        ...
        public List<object> GetStockPackagingActual(int Tipelokasi, int IDLokasi, int Packaging)
        {
            List<object> datas = new List<object>();
            ...
            var actual = _context.StockActualPackagings
                    ...
                    && (p.Idproduct == Packaging || (p.Idproduct > 0 && Packaging == 0))
                    && p.Packaging.Disable == false && p.Packaging.Draft == false).ToList();
            ...
            datas = (from a in actual
                     ...
                     select new
                    {
                        Packaging = a.Packaging.Nama,
                        Kode = a.Packaging.Kode,
                        Inhand = a.InHand,
                        InTransit = a.InTransit.HasValue ? a.InTransit : 0,
                        Damaged = a.Damaged.HasValue ? a.Damaged : 0,
                        Lokasi = location.SingleOrDefault(p => p.IDLokasi == a.Idlokasi && p.TipeLokasi == a.TipeLokasi).Nama
        ...
        public List<object> GetPackaging()
        {
            List<object> datas = new List<object>();
            datas = (from a in _context.Packagings.Where(p => p.Disable == false && p.Draft == false)
                     select new {
                        ID = a.Id,
                        Nama = a.Nama,
                        Kode = a.Kode
        ...
        public object GetDetailPackaging(int ID)
        {
            object datas = (from a in _context.Packagings.Where(p => p.Id == ID)
                            select new
                            {
                                ID = a.Id,
                                Nama = a.Nama,
        ...
        public List<object> GetListPackaging(string Keyword, int Status)
        {
            List<object> datas = new List<object>();
            datas = (from a in _context.Packagings.Where(p => p.Draft == false && ((Status > 0 && p.Disable == GetStatus(Status)) || Status == 0) && (p.Nama.Contains(Keyword) || p.Kode.Contains(Keyword)))
                     select new
                     {
                         ID = a.Id,
                         Nama = a.Nama,
        ...
        public void InsertPackaging(PCS data)
        {
            using (var tempContext = _context.Database.BeginTransaction())
            {
                try
                {
                    Packaging product = new Packaging();
                    ...
                    if (data.files != null) common.UploadImage(data.files, connectionStrings.AppConfig.DirPackaging, ref filename);

                    product.ImgPicture = filename;

                    _context.Packagings.Add(product);
        ...
        public void UpdatePackaging(PCS data, string UserName)
        {
            using (var tempContext = _context.Database.BeginTransaction())
            {
                try
                {
                    var product = _context.Packagings.SingleOrDefault(p => p.Id == data.ID);
                    ...
                    if (product.Harga != data.Harga)
                    {
                        ...
                        _context.PackagingPricingLogs.Add(log);
                    }
                    product.Harga = data.Harga;
                    if (data.files != null)
                    {
                        ...
                        common.UploadImage(data.files, connectionStrings.AppConfig.DirPackaging, ref filename);
        ...
        #region Receive
        public object GetReceivePackaging(int IDLokasi, string From, string To, bool IsApproval)
        {
            List<object> datas = new();

            string query = "exec sp_ReceivedPackaging " +
                IDLokasi + ", " +
                "'" + From + "', " +
                "'" + To + "'";
        ...
        public object GetDetailReceivePackaging(int ID, int IDLokasi, int IDProduct, string From, string To, int IsSummary, bool IsApproval)
        {
            From = String.IsNullOrEmpty(From) ? "1900-01-01" : From;
            To = String.IsNullOrEmpty(To) ? "1900-01-01" : To;

            var data = _context.StockReceivePackagings.Include(p => p.LocWarehouse).Include(p => p.StockReceivePackaging_Products).ThenInclude(p => p.Packaging)
                           .Where(p => (p.Id == ID)
                           ...
                           && (p.StockReceivePackaging_Products.Any(q => q.Idproduct == IDProduct) || (IDProduct == 0))));
            ...
            foreach (var x in data)
            {
                datas.Add(new
                {
                    ...
                    Detail = (from a in x.StockReceivePackaging_Products
                               select new
                               {
                                   ID = a.Idproduct,
                                   Nama = a.Packaging.Nama,
                                   Kode = a.Packaging.Kode,
                                   Qty = a.Qty,
        ...
        private object GetReceiveProductPackaging(int ID, int IDProduct)
        {
            object datas = (from a in _context.StockReceivePackagingProducts.Include(p => p.Packaging).Where(p => p.Idform == ID && (p.Idproduct == IDProduct|| (IDProduct == 0 && p.Idproduct > 0)))
                            select new
                            {
                                Nama = a.Packaging.Nama,
                                Kode = a.Packaging.Kode,
        ...
        public void InsertReceivingPackaging(ReceivePCS Data, string UserName)
        {
            using (var tempContext = _context.Database.BeginTransaction())
            {
                try
                {
                    StockReceivePackaging receive = new StockReceivePackaging();
                    ...
                    _context.StockReceivePackagings.Add(receive);
                    _context.SaveChanges();

                    foreach (PCS x in Data.Product)
                    {
                        StockReceivePackaging_Product product = new StockReceivePackaging_Product();
                        product.Idform = receive.Id;
                        product.Idproduct = x.ID;
                        product.Qty = x.Qty;
                        product.Harga = 0;

                        _context.StockReceivePackagingProducts.Add(product);
                    }
        ...
        public void ApproveReceivingPackaging(ReceivePCS Data, string UserName)
        {
            using (var tempContext = _context.Database.BeginTransaction())
            {
                try
                {
                    StockReceivePackaging receive = _context.StockReceivePackagings.SingleOrDefault(p => p.Id == Data.ID);
                    ...
                    foreach (PCS x in Data.Product)
                    {
                        StockReceivePackaging_Product product = _context.StockReceivePackagingProducts.Include(p => p.Packaging).SingleOrDefault(p => p.Idform == Data.ID && p.Idproduct == x.ID);

                        product.Received = x.Received;
                        product.Damaged = x.Damaged;
                        product.NeverArrived = x.NeverArrived;

                        if (_context.StockActualPackagings.Any(p => p.TipeLokasi == 0 && p.Idlokasi == receive.Idwarehouse && p.Idproduct == x.ID))
                        {
                            StockActualPackaging actual = _context.StockActualPackagings.SingleOrDefault(p => p.TipeLokasi == 0 && p.Idlokasi == receive.Idwarehouse && p.Idproduct == x.ID);

                            actual.InHand += x.Received;
                        }
                        else
                        {
                            StockActualPackaging actual = new StockActualPackaging();

                            actual.Idlokasi = receive.Idwarehouse;
                            actual.TipeLokasi = 0;
                            actual.Idproduct = x.ID;
                            actual.Nomor = product.Packaging.Kode;
                            ...
                            _context.StockActualPackagings.Add(actual);
                        }
                    }
        ...
        public void DeleteReceivingPackaging(int ID)
        {
            using (var tempContext = _context.Database.BeginTransaction())
            {
                try
                {
                    List<StockReceivePackaging_Product> products = _context.StockReceivePackagingProducts.Where(p => p.Idform == ID).ToList();
                    _context.StockReceivePackagingProducts.RemoveRange(products);

                    StockReceivePackaging receive = _context.StockReceivePackagings.SingleOrDefault(p => p.Id == ID);
                    _context.StockReceivePackagings.Remove(receive);
        ...
        public void DeleteReceivedPackaging(int ID)
        {
            using (var tempContext = _context.Database.BeginTransaction())
            {
                try
                {
                    ...
                    foreach (StockReceivePackaging_Product x in products)
                    {
                        StockActualPackaging actual = _context.StockActualPackagings.SingleOrDefault(p => p.TipeLokasi == 0 && p.Idlokasi == receive.Idwarehouse && p.Idproduct == x.Idproduct);
        ...
        public object GetPemakaianPackaging(int Tipelokasi, int IDLokasi, int Packaging, string From, string To, string Keyword = "")
        {
            List<object> datas = new();
            Keyword = String.IsNullOrEmpty(Keyword) ? "" : Keyword;

            var data = _context.SalesOrders.Include(p => p.SalesOrderPackagings)
                ...
                && p.SalesOrderPackagings.Any(q => q.Idproduct == Packaging || (Packaging == 0 && q.Idproduct > 0)));
            ...
            foreach (var x in data)
            {
                ...
                datas.Add(new
                {
                    ...
                    Lokasi = Lokasi,
                    Detail = (from a in x.SalesOrderPackagings
                              where (a.Idproduct == Packaging || (Packaging == 0 && a.Idproduct > 0))
                              select new
                              {
                                  Invoice = a.Invoice,
                                  Product = a.Packaging.Nama,
                                  Kode = a.Packaging.Kode,
                                  Qty = a.Qty,
