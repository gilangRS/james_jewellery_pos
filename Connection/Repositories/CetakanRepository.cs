...
{
    public class CetakanRepository : ICetakanRepository
    {
        ...
        public CetakanRepository()
        {
            _context = new JAWSDbContext();
            this.connectionStrings = new ConnectionString();
        }

        public List<object> GetStockCetakanSummary(int Tipelokasi, int IDLokasi, int Packaging, string From, string To, int Groupby, bool IsStore, string NIK)
        {
            List<object> datas = new List<object>();
            string query = "exec sp_summary_cetakan "
                + Tipelokasi + ", '"
                + From + "', '"
                + To + "', "
                + IDLokasi + ", "
        ...
        public List<object> GetStockCetakanActual(int Tipelokasi, int IDLokasi, int Packaging)
        {
            List<object> datas = new List<object>();
            ...
            var actual = _context.StockActualCetakans
                    .Include(p => p.Cetakan)
                    ...
                    && p.Cetakan.Disable == false && p.Cetakan.Draft == false).ToList();
        ...
        public List<object> GetCetakan()
        {
            List<object> datas = new List<object>();
            datas = (from a in _context.Cetakans.Where(p => p.Disable == false && p.Draft == false)
                     select new
                     {
                         ID = a.Id,
                         Nama = a.Nama,
        ...
        public List<object> GetListCetakan(string Keyword, int Status)
        {
            List<object> datas = new List<object>();
            datas = (from a in _context.Cetakans.Where(p => p.Draft == false && ((Status > 0 && p.Disable == GetStatus(Status)) || Status == 0) && (p.Nama.Contains(Keyword) || p.Kode.Contains(Keyword)))
                     select new
                     {
                         ID = a.Id,
                         Nama = a.Nama,
        ...
        public object GetDetailCetakan(int ID)
        {
            object datas = (from a in _context.Cetakans.Where(p => p.Id == ID)
                            select new
                            {
                                ID = a.Id,
                                Nama = a.Nama,
        ...
        public void InsertCetakan(PCS data)
        {
            using (var tempContext = _context.Database.BeginTransaction())
            {
                try
                {
                    Cetakan product = new Cetakan();
                    ...
                    product.Draft = false;
                    product.ImgPicture = connectionStrings.AppConfig.DirCetakan + data.Kode + extension;
                    product.Harga = data.Harga;


                    string filename = data.Kode;

                    if(data.files != null) common.UploadImage(data.files, connectionStrings.AppConfig.DirCetakan, ref filename);

                    product.ImgPicture = filename;

                    _context.Cetakans.Add(product);
        ...
        public void UpdateCetakan(PCS data, string UserName)
        {
            using (var tempContext = _context.Database.BeginTransaction())
            {
                try
                {
                    var product = _context.Cetakans.SingleOrDefault(p => p.Id == data.ID);
                    ...
                    if (product.Harga != data.Harga)
                    {
                        ...
                        _context.CetakanPricingLogs.Add(log);
                    }
                    product.Harga = data.Harga;
                    ...
                    if (data.files != null)
                    {
                        ...
                        common.UploadImage(data.files, connectionStrings.AppConfig.DirCetakan, ref filename);
        ...
        #region Receive
        public object GetReceiveCetakan(int IDLokasi, string From, string To, bool IsApproval)
        {
            List<object> datas = new();

            string query = "exec sp_ReceivedCetakan " + 
                IDLokasi + ", " +
                "'" + From + "', " +
                "'" + To + "'";
        ...
        public object GetDetailReceiveCetakan(int ID, int IDLokasi, int IDProduct, string From, string To, int IsSummary, bool IsApproval)
        {
            From = String.IsNullOrEmpty(From) ? "1900-01-01" : From;
            To = String.IsNullOrEmpty(To) ? "1900-01-01" : To;

            var data = _context.StockReceiveCetakans.Include(p => p.LocWarehouse).Include(p => p.StockReceiveCetakan_Products).ThenInclude(p => p.Cetakan)
                           .Where(p => (p.Id == ID)
                           ...
                           && (p.StockReceiveCetakan_Products.Any(q => q.Idproduct == IDProduct) || (IDProduct == 0))));
            ...
            foreach (var x in data)
            {
                datas.Add(new
                {
                    ...
                    Detail = (from a in x.StockReceiveCetakan_Products
                              select new
                              {
                                  ID = a.Idproduct,
                                  Nama = a.Cetakan.Nama,
        ...
        private object GetReceiveProductCetakan(int ID)
        {
            object datas = (from a in _context.StockReceiveCetakanProducts.Include(p => p.Cetakan).Where(p => p.Idform == ID)
                            select new
                            {
                                Nama = a.Cetakan.Nama,
                                Kode = a.Cetakan.Kode, 
        ...
        public void InsertReceivingCetakan(ReceivePCS Data, string UserName)
        {
            using (var tempContext = _context.Database.BeginTransaction())
            {
                try
                {
                    StockReceiveCetakan receive = new StockReceiveCetakan();
                    ...
                    _context.StockReceiveCetakans.Add(receive);
                    _context.SaveChanges();

                    foreach (PCS x in Data.Product)
                    {
                        StockReceiveCetakan_Product product = new StockReceiveCetakan_Product();
                        product.Idform = receive.Id;
                        product.Idproduct = x.ID;
                        product.Qty = x.Qty;
                        product.Harga = 0;

                        _context.StockReceiveCetakanProducts.Add(product);
                    }
        ...
        public void ApproveReceivingCetakan(ReceivePCS Data, string UserName)
        {
            using (var tempContext = _context.Database.BeginTransaction())
            {
                try
                {
                    StockReceiveCetakan receive = _context.StockReceiveCetakans.SingleOrDefault(p => p.Id == Data.ID);
                    ...
                    foreach (PCS x in Data.Product)
                    {
                        StockReceiveCetakan_Product product = _context.StockReceiveCetakanProducts.Include(p => p.Cetakan).SingleOrDefault(p => p.Idform == Data.ID && p.Idproduct == x.ID);

                        product.Received = x.Received;
                        product.Damaged = x.Damaged;
                        product.NeverArrived = x.NeverArrived;

                        if(_context.StockActualCetakans.Any(p => p.TipeLokasi == 0 && p.Idlokasi == receive.Idwarehouse && p.Idproduct == x.ID))
                        {
                            StockActualCetakan actual = _context.StockActualCetakans.SingleOrDefault(p => p.TipeLokasi == 0 && p.Idlokasi == receive.Idwarehouse && p.Idproduct == x.ID);

                            actual.InHand += x.Received;
                        }
                        else
                        {
                            StockActualCetakan actual = new StockActualCetakan();

                            actual.Idlokasi = receive.Idwarehouse;
                            actual.TipeLokasi = 0;
                            actual.Idproduct = x.ID;
                            actual.Nomor = product.Cetakan.Kode;
                            ...
                            _context.StockActualCetakans.Add(actual);
                        }
                    }
        ...
        public void DeleteReceivingCetakan(int ID)
        {
            using (var tempContext = _context.Database.BeginTransaction())
            {
                try
                {
                    List<StockReceiveCetakan_Product> products = _context.StockReceiveCetakanProducts.Where(p => p.Idform == ID).ToList();
                    _context.StockReceiveCetakanProducts.RemoveRange(products);

                    StockReceiveCetakan receive = _context.StockReceiveCetakans.SingleOrDefault(p => p.Id == ID);
                    _context.StockReceiveCetakans.Remove(receive);
        ...
        public void DeleteReceivedCetakan(int ID)
        {
            using (var tempContext = _context.Database.BeginTransaction())
            {
                try
                {
                    ...
                    foreach (StockReceiveCetakan_Product x in products)
                    {
                        StockActualCetakan actual = _context.StockActualCetakans.SingleOrDefault(p => p.TipeLokasi == 0 && p.Idlokasi == receive.Idwarehouse && p.Idproduct == x.Idproduct);
        ...
        public object GetPemakaianCetakan(int Tipelokasi, int IDLokasi, int Cetakan, string From, string To, string Keyword = "")
        {
            List<object> datas = new();
            Keyword = String.IsNullOrEmpty(Keyword) ? "" : Keyword;

            var data = _context.SalesOrders.Include(p => p.SalesOrderCetakans)
                .ThenInclude(p => p.Cetakan)
                ...
                && p.SalesOrderCetakans.Any(q => q.Idproduct == Cetakan || (Cetakan == 0 && q.Idproduct > 0)));
            ...
            foreach (var x in data)
            {
                ...
                datas.Add(new
                {
                    ...
                    Lokasi = Lokasi,
                    Detail = (from a in x.SalesOrderCetakans
                              where (a.Idproduct == Cetakan || (Cetakan == 0 && a.Idproduct > 0))
                              select new
                              {
                                  Invoice = a.Invoice,
                                  Product = a.Cetakan.Nama,
                                  Kode = a.Cetakan.Kode,
                                  Qty = a.Qty,
